...
for i in range(M):
    ...
    if Sqlt[i]>3 and (Wqlt[i]>3 or Wqlt[i]<0) and inc_flg[i]==0:
        flag[i] = 0
...
#for i in range(len(r_mag)):
    ...
    #dg  = np.abs(g_mag[i]-g_mag_prime[i])
    #dr  = np.abs(r_mag[i]-r_mag_prime[i])
    #di  = np.abs(i_mag[i]-i_mag_prime[i])
    #dw1 = np.abs(w1_mag[i]-w1_mag_prime[i])
    #dw2 = np.abs(w2_mag[i]-w2_mag_prime[i])

    #if (dg>0.15 and dg<1.) or (dr>0.15 and dr<1.) or (di>0.15 and di<1.) or (dw1>0.15 and dw1<1.) or (dw2>0.15 and dw2<1.):
        #print pgc_ESN[i], '%.2f'%dg, '%.2f'%dr, '%.2f'%di, '%.2f'%dw1, '%.2f'%dw2
        #N+=1
