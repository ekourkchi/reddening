...
def lnlike(theta, inc, R, pc0, Epc0, noise2):
    ...
    K = kernel(X, X, [theta[0], theta[1]], sigma_f=theta[2])+ noise2
...
def lnprior(theta):
    ...
    if l1>100. or l1<-100: return -np.inf 
    if l2>100. or l2<-100: return -np.inf 
    if sigma>100. or sigma<-100: return -np.inf 
...
r_w1    = Input[1][0:700]
pc0     = Input[2][0:700]
inc     = np.cos(Input[3][0:700]*np.pi/180.)
...
Er_w1 = table['Er_w1'][0:700]
Epc0  = table['Epc0'][0:700]
Einc  = table['inc_e'][0:700]
...
### Maximum Likelihood
#result = minimize(nll_fn2(X, y, Epc0, noise2), [0, 0, 1, 1, 1], 
               #bounds=((-10, 10), (-10, 10), (1e-5, 20), (1e-5, 20), (1e-5, 10)),
               #method='L-BFGS-B')
#print result
...
if True:    ## MCMC part
        ...
        fig = corner.corner(samples, labels=["l1", "l2", r"$\sigma_f$"], truths=truths, truth_color='r', quantiles=[0.16, 0.84],
