...
def plot_array(inFile, scatter=False, binned=True):
    ...
    fig = py.figure(figsize=(16, 5.5), dpi=100)    
    fig.subplots_adjust(wspace=0, top=0.98, bottom=0.12, left=0.06, right=0.99)
    ...
    for band in band_lst:
        ...
        ax = plt.subplot(gs[p]) ; p+=1
        plot_Rinc_(ax, T2[band], Input2[band], color=dye[band], scatter=scatter, binned=binned, xlabel=xlabel, ylabel=ylabel, band=band)
        yticks = ax.yaxis.get_major_ticks()
        if band!='u': yticks[-1].label1.set_visible(False)
        if band!='u': plt.setp(ax.get_yticklabels(), visible=False)
    #####################################################
    ...
    ax.annotate(r'$\langle \mu_2 \rangle^{(i)}_e \/\/ [mag \/ arcsec^2]$', (0.45,0.02), xycoords='figure fraction', size=16, color='black')
    
    fig.savefig("gamma_surfB2_lambda.eps")
    fig.savefig("gamma_surfB2_lambda.png")
    plt.show()
...
###########################################################
def plot_Rinc_(ax, T2, Input2, color='red', scatter=False, binned=False, xlabel=True, ylabel=True, X_twin=True, Y_twin=True, band='r'):
    ...
    if scatter:
        ...
        ax.text(24.5,-0.4, r'$RMS=$'+'%.2f'%rms+' mag', fontsize=12, color='k')
