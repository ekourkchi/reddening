    ...
    **hist2d_kwargs
    ...
    if truths is not None:
        try:
            ...
            truths_ = np.zeros_like(truths)
        except:
            print('Warning: could not convert truths values to a numpy array ...!')
    ...
    for i, x in enumerate(xs):
        ...
        # Plot the histograms.
        if smooth1d is None:
            n, pp, _ = ax.hist(x, bins=bins[i], weights=weights,
                              range=np.sort(range[i]), **hist_kwargs)
            
            indx, = np.where(n==max(n))
            indx=indx[0]
            truths_[i] = 0.5*(pp[indx]+pp[indx+1])
            print("Max of the histogram, i,max: ",i, truths_[i])
        ...
        if truths is not None and truths[i] is not None:
            ...
            if truths_ is not None: ax.axvline(truths_[i], color='green', linestyle=":")
        ...
        if show_titles:
            ...
            if title_fmt is not None:
                # Compute the quantiles for the title. This might greeno
        ...
        for j, y in enumerate(xs):
            ...
            if truths is not None:
                if truths[j] is not None:
                    ...
                    #if truths_ is not None: ax.axvline(truths_[j], color='green')
                if truths[i] is not None:
                    ax.axhline(truths[i], color=truth_color)
                    #if truths_ is not None: ax.axhline(truths_[i], color='green')
                if truths[i] is not None and truths[j] is not None:
                    ax.plot(truths[j], truths[i], "s", color=truth_color)
                    ax.plot(truths_[j], truths_[i], "o", color='green')
